...
@Composable
fun RequestPermissionOverlayScreen(
    ...
    OnboardingLayout(onBack = onBack) {
        ...
        OnboardingTitle(stringResource(R.string.onboarding_overlay_permission_title))
        Spacer(modifier = Modifier.height(BeeSpacing.XL))
        BeeBodyLarge(
            text = stringResource(R.string.onboarding_overlay_permission_body),
        ...
        if (isOverlayEnabled) {
            BeeButtonSecondary(
                ...
                BeeLabelLarge(stringResource(R.string.onboarding_overlay_permission_active))
            }
        } else {
            BeeButtonPrimary(
                ...
                BeeLabelLarge(stringResource(R.string.onboarding_overlay_permission_enable), color = MaterialTheme.colorScheme.onPrimary)
            }
        }

        Spacer(modifier = Modifier.height(BeeSpacing.M))
        OnboardingNavigationRow(onNext = onNext, nextEnabled = isOverlayEnabled)
    }
}
